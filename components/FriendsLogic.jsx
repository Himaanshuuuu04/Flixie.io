...
export default function FriendsLogic() {
  ...
  const searchedUsers = useSelector((state) => state.friend.searchedUsers);
  const friendRequests = useSelector((state) => state.friend.friendRequests);
  const userLoading = useSelector((state) => state.friend.userLoading);
  ...
  const handleRequestAction = async (requestId, action) => {
    ...
    try {
      console.log(
        `Handling friend request ${action} for request ID ${requestId}`,
      );
      await dispatch(handleFriendRequest({ requestId, action }));
    } catch (error) {
  ...
  return (
    <div className="text-white p-6 h-full bg-white/5 border border-white/20 backdrop-blur-sm rounded-2xl shadow-lg">
      <h2 className="md:text-4xl text-2xl font-semibold mb-6 mt-4 text-center">
        Friends
      </h2>
                ...
                >
                  <div className="flex items-center">
                    <img
                      ...
                      height={40}
                      unoptimized
                      className="rounded-full mr-3 border border-white/50"
                    />
                    <div>
        ...
        <form onSubmit={handleSearch} className="w-full xl:w-1/2 mb-8 mx-auto">
          <label className="flex flex-col gap-3">
            ...
            <div className="flex gap-4 relative">
              {" "}
              {/* Make the parent relative for positioning the cross button */}
              <input
                type="text"
                value={query}
          ...
          {searchedUsers.length > 0 && (
            <ul className="mt-4">
              {searchedUsers.map((user) => (
                <li
                  key={user.$id}
                  className="flex items-center justify-between p-3 border border-white/20 rounded-xl mb-2 bg-white/10"
                >
                  <div className="flex items-center">
                    <Image
              ...
              friendRequests.map((request) => {
                if (
                  ...
                  return (
                    <li
                      key={request.$id}
                      className="flex items-center justify-between p-3 border rounded-xl mb-2 bg-white/10"
                    >
                      <div>
