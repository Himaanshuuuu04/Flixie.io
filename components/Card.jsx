...
export default function Card({
  ...
  return (
    <Link
      href={`/Moviedetails/${media}/${id}`}
      className="flex flex-col items-center cursor-pointer w-fit"
    >
      <div className="relative group border border-white/30 rounded-xl backdrop-filter backdrop-blur-xl text-white transition-all duration-300 hover:bg-white/30 hover:scale-110 shadow-xl w-36 h-52 sm:w-44 sm:h-64 md:w-36 md:h-52">
        <Image
          src={img}
          alt={title || "movie poster"}
          fill
          sizes="(max-width: 768px) 11rem, 9rem"
          className="rounded-xl object-cover opacity-85 transition-all duration-300 group-hover:blur-sm"
        />
        {/* Overlay */}
