...
const Carousel = () => {
  ...
  const { carouselMovies, loading } = useSelector(
    (state) => state.search,
  ...
  const playSrc = Play?.src || Play;
  const favoriteSrc = Favorite?.src || Favorite;
      ...
      >
        ...
        >
          {movies.map((movie) => (
            <div
              key={movie.id}
              className="flex-shrink-0 w-full h-[13rem] relative"
            >
              <Link href={`/Moviedetails/${movie.media_type}/${movie.id}`}>
                  ...
                  <div className="text-white text-center space-y-2">
                    ...
                    <div className="absolute bottom-5 left-5 text-left backdrop:filter backdrop-blur-3xl p-4 rounded-3xl border border-white/20 bg-black/50 mr-5 shadow-2xl">
                      ...
                      <div className="flex gap-2 mt-2">
                        ...
                        >
                          <img src={playSrc} alt="Play" className="w-4 h-4" />{" "}
                          <span className="-mb-1">Watch Now</span>
                        </button>

                        <button
                          className="text-xs hover:bg-red-500 focus:scale-105 border-white/20 border hover:border-red-500 transition-all duration-300 px-2 py-1 rounded-full flex items-center space-x-1"
                          aria-label="Like"
                        >
                          <img
                            src={favoriteSrc}
                ...
                <div className="absolute bottom-0 h-[40%] bg-black/50 w-full flex flex-col justify-center p-4 md:hidden backdrop-blur-xl rounded-xl border border-white/20 rounded-t-none text-white transition-opacity duration-300 ">
                  <h2 className="font-light text-nowrap text-lg mb-1">
                    {movie.title || movie.name} -{" "}
                    {movie.release_date || movie.first_air_date}
                  </h2>
                  <div className="flex gap-4">
                    <button
                      className="text-sm border font-extralight  bg-white/10 border-white/20 px-2 py-[0.20rem] rounded-lg flex items-center space-x-1"
                      aria-label="Watch Now"
                    >
                      <img src={playSrc} alt="Play" className="w-4 h-4" />{" "}
                      <span>Watch Now</span>
                    ...
                    <button
                      className="text-sm border font-extralight bg-white/10 border-white/20 px-2 rounded-lg flex items-center space-x-1"
                      aria-label="Like"
                    >
                      <img
                        src={favoriteSrc}
      ...
      <div className="mt-2 md:mt-4 flex justify-center space-x-1">
        {movies.map((_, index) => (
          <button
            ...
            className={`h-1.5 w-1.5 md:w-2 md:h-2 rounded-full transition duration-300 ease-in-out ${currentIndex === index ? "bg-white" : "bg-white/10"}`}
          />
        ))}
      </div>
    </div>
